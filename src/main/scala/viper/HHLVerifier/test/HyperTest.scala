...
object HyperTest {
  ...
  def evaluateResult(result : TestResult,  test : (File, TestResult)) = {
    (result, test._2) match {
      case (TypeCheckSuccess(VerificationSuccess()), TypeCheckSuccess(VerificationSuccess())) |
            (TypeCheckSuccess(VerificationFailure()), TypeCheckSuccess(VerificationFailure())) |
            (TypeCheckSuccess(_), TypeCheckSuccess(UnknownVerification())) |
            (TypeCheckFailure(_), TypeCheckFailure(_)) => {
