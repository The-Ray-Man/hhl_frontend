...
/** The Parser object
  *
  * This class contains all parsing rules written in fastparse. Calling parse on program will create an AST or result in a parsing error.
  *
...
object Parser {
  ...
  /** Program variables
    *
    * Program variables to refer to all variables which have assigned values in the program code. Use case: {{{var num: Int}}}
    */
  ...
  /** Assert variables
    *
    * Assert variables are used in assertions to describe states. They can be used only in assertions! Use case: {{{hyperAssert forall <_s> :: _s[num] == 2}}}
    */
  ...
  /** Proof variables
    *
    * TODO: Insert description. Declared with let ..., start with $, used in statements.
    */
  ...
  /** Statements
    *
    * Statements are the basic building blocks of every Hypra program. They are used to perform actions which change program states.
    */
  ...
  ).map { case (oL, stmt, oR) => mapStmt(oL, stmt, oR) }
  ...
  def basicExpr[$: P]: P[Expr] = P(Index ~ (compositeTypeAssign | lengthExpr | loopIndex | proofVar | boolean | unaryExpr | useHint | identifier | number | "(" ~ expr ~ ")") ~ Index).map { case (oL, expr, oR) =>
    expr.setOffsets(oL, oR)
  }
