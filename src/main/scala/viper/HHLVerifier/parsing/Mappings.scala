...
/** Parser Mappings
  *
  * This file is a companion object for [[viper.HHLVerifier.parsing.Parser]]. It contains all functions which map parser rules to actual AST objects.
  *
  * In order to understand the functionality of those, take a look at the parser ruler. They should contain documentation for the individual object.
  */
object Mappings {
  ...
  def mapWhileLoop(items: (String, Expr, Seq[(Option[HintDecl], Expr)], Option[Expr], CompositeStmt)): WhileLoopStmt = {
    val rule =
      if (items._1 == "" && !Generator.autoSelectRules) throw UnknownException("Each while loop must be specified with exactly one rule unless auto-selection of rules is turned on. ")
      else if (items._1 == "") "unspecified"
      else items._1
    val cond = items._2
