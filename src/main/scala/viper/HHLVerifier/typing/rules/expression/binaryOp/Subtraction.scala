...
case class SubtractionCombineFunctionHypertype(override val rules: Seq[RuleWrapper[HyperTypeConclusion]] = Seq()) extends binaryCombineFunction[HyperTypeConclusion](rules) {}

case class SubtractionCombineFunctionDeltatype(override val rules: Seq[RuleWrapper[DeltaConclusion]] = Seq()) extends binaryCombineFunction[DeltaConclusion](rules) {}

case class SubtractionDerivationRule(val combineFunctionHypertype: binaryCombineFunction[HyperTypeConclusion] = SubtractionCombineFunctionHypertype(), val combineFunctionDelta: binaryCombineFunction[DeltaConclusion] = SubtractionCombineFunctionDeltatype()) extends ExpressionDerivationRule {
