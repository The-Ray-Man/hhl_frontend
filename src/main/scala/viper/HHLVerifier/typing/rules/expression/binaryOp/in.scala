...
case class InCombineFunctionHypertype(override val rules: Seq[RuleWrapper[HyperTypeConclusion]] = Seq()) extends binaryCombineFunction[HyperTypeConclusion](rules) {}

case class InCombineFunctionDeltatype(override val rules: Seq[RuleWrapper[DeltaConclusion]] = Seq()) extends binaryCombineFunction[DeltaConclusion](rules) {}

case class InDerivationRule(val combineFunctionHypertype: binaryCombineFunction[HyperTypeConclusion] = InCombineFunctionHypertype(), val combineFunctionDelta: binaryCombineFunction[DeltaConclusion] = InCombineFunctionDeltatype()) extends ExpressionDerivationRule {
