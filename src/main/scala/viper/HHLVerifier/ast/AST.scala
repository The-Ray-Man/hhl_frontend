...
sealed class Expr() extends ErrorData {
  ...
  /** Base Type of the expression. Only set for composite objects.
    *
    * The exact meaning of this type is tricky. It is used when further information is needed to translate operations with composite objects. For example, for intSeq[0] the type is Int, while the base type is Seq[Int]
    */
...
/** Contains an encoded assertion with a quantifier, assertion variable declarations and an expression
  *
  * @param quantifier
  *   quantitifer encoded as a string
  * @param assertVarDecls
  *   list of assert variable declarations
  * @param body
  *   expression
  */
...
/** Encodes a call to a method
  ...
  *   method identifier
  * @param args
  *   identifiers of Hypra variables
  */
...
case class CompositeStmt(stmts: Seq[Stmt]) extends Stmt {
  ...
  /** This map stores all the program variables used in this CompositeStmt object It is filled in the SymbolChecker Used as arguments when creating the method to verify a loop invariant
    */
