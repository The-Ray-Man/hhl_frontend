  ...
  def checkSymbolsExpr(exp: Expr, isInLoopInv: Boolean, isFrame: Boolean): Seq[(String, Type)] = {
    exp match {
      ...
      case SeqAssignExpr(elements) =>
        elements.foldRight(Seq.empty[(String, Type)]) { case (l, r) =>
          r ++ checkSymbolsExpr(l, isInLoopInv, isFrame)
        }
      ...
      case MapAssignExpr(elements) =>
        elements.foldRight(Seq.empty[(String, Type)]) { case (l, r) =>
          r ++ checkSymbolsExpr(l.k, isInLoopInv, isFrame) ++ checkSymbolsExpr(l.v, isInLoopInv, isFrame)
        }
