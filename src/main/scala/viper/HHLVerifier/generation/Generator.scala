...
/** Documentation of Error Handling
  ...
  *   - There is a specific error generator class, which captures the faulty expression, wraps it into a nice error encodes it into the hypra encoding
  *
  * For all errors which include invariants
  *   - Invariants are transformed multiple times in the process. Therefore, we need to track which transformed invariant belongs to which original one. This is achieved by using the invariant tracking object.
  *   - Every invariant is added to the tracker when it is first discovered
  *   - Additionally, when an invariant is transformed by a quantifier removal, the removed quantifier count is updated to allow to inform the user about the state of the expression which caused the error
  */
...
object Generator {
  ...
  def translateMethod(method: Method): Unit = {
    ...
    val assumeSFailEmpty = vpr.Inhale(
      vpr.Forall(
        Seq(state),
        Seq.empty,
    ...
    val posts = method.post.map { exp =>
      ...
      translateExp(
        normalizedPost,
        null,
        outputStates.localVar,
        outputFailureStates.localVar,
        info = new Logger(VerificationErrors.Postcondition(exp), Logger.ERR)
  ...
  def translateStmt(stmt: Stmt, currStates: vpr.LocalVar, currFailureStates: vpr.LocalVar, isAutoSelected: Boolean = false): (Seq[vpr.Stmt], Seq[vpr.LocalVar]) = {
    ...
    stmt match {
      ...
      case ProofVarDecl(pv, p) =>
        ...
        val assertVarExists = vpr.Assert(vpr.Exists(Seq(getAliasForProofVar(pv)), Seq.empty, translateExp(p, state, currStates, currFailureStates))())(info =
          new Logger(VerificationErrors.Deprecated(p), Logger.ERR)
            .addTitle("Verification Error")
      ...
      case AssumeStmt(e) =>
        ...
        if (verifierOption != 1) {
          ...
          val exp = vpr.And(SetState.getInSetApp(Seq(state, currStates)), translateExp(e, state, currStates, currFailureStates))()
      ...
      case AssertStmt(e) =>
        ...
        val updateSFail      = vpr.LocalVarAssign(currFailureStates, SetState.getSetUnionApp(Seq(currFailureStates, tempFailedStates.localVar)))()

        if (verifierOption != 1) {
          // ForAll
          val exp1               = vpr.And(SetState.getInSetApp(Seq(state, currStates)), translateExp(e, state, currStates, currFailureStates))()
          val exp2               = vpr.And(SetState.getInSetApp(Seq(state, currStates)), translateExp(UnaryExpr("!", e), state, currStates, currFailureStates))()
      ...
      case stmt @ HyperAssertStmt(e) =>
        val assert = vpr.Assert(translateExp(e, null, currStates, currFailureStates))(info =
          new Logger(VerificationErrors.HyperAssertion(e), Logger.ERR)
            .addTitle("Verification Error")
            .addOffset((stmt.offsetLeft, stmt.offsetRight))
            .toAnnotationInfo()
        )
      ...
      case MultiAssignStmt(left, right) =>
        ...
        if (callee.pre.nonEmpty) {
          ...
          callee.pre.foreach { exp =>
            ...
            newStmts = newStmts :+ vpr.Assert(vprExp)(info =
              new Logger(VerificationErrors.MethodCall(exp), Logger.ERR)
                .addTitle("Verification Error")
                .addOffset((exp.offsetLeft, exp.offsetRight))
                .toAnnotationInfo()
            )
        ...
        // Update S and S_fail
        val updateSFail = vpr.LocalVarAssign(currFailureStates, SetState.getSetUnionApp(Seq(currFailureStates, tempFailedStates)))()
      ...
      case HavocStmt(id, hintDecl) =>
        ...
        if (verifierOption != 0) {
          ...
          val stmt2            = translateHavocVarHelper(
            currStates,
            STmp,
            state,
            s1,
      ...
      case IfElseStmt(cond, ifStmt, elseStmt) =>
        ...
        if (declareBlock != null) {
          ...
          if (verifierOption != 0) {
            setFlagForIf = setFlagForIf :+ vpr.Inhale(vpr.Forall(Seq(stateDecl), Seq.empty, vpr.Implies(SetState.getInSetApp(Seq(state, ifBlockStates), useForAll = false), vpr.EqCmp(State.get(state, isIfBlock), one)())())())()
            setFlagForElse = setFlagForElse :+ vpr.Inhale(
              vpr.Forall(
                Seq(stateDecl),
                Seq.empty,
                vpr.Implies(SetState.getInSetApp(Seq(state, elseBlockStates), useForAll = false), vpr.EqCmp(State.get(state, isIfBlock), zero)())()
              )()
            )()
          }
            ...
            resumeIfBlockStates = resumeIfBlockStates :+
              vpr.Inhale(
                vpr.Forall(
                  Seq(stateDecl),
                  Seq.empty,
                  vpr.Implies(SetState.getInSetApp(Seq(state, STmp)), vpr.And(SetState.getInSetApp(Seq(state, currStates)), vpr.EqCmp(State.get(state, isIfBlock), one)())())()
                )()
              )()
            resumeElseBlockStates = resumeElseBlockStates :+
              vpr.Inhale(
                vpr.Forall(
                  ...
                  Seq.empty,
                  vpr.Implies(
                    SetState.getInSetApp(Seq(state, STmp), false),
                    vpr.And(SetState.getInSetApp(Seq(state, currStates), false), vpr.EqCmp(State.get(state, isIfBlock), zero)())()
                  )()
                )()
              )()
          }
      ...
      case loop @ WhileLoopStmt(cond, body, invWithHints, decr, rule) =>
        ...
        if (autoSelectRules && rule == "unspecified") {
          ...
          if (!isAutoSelected && !syncTotWarningPrinted && postIsTopExists && rule != "syncTotRule" && rule != "existsRule") {
            new Logger(
              "Warning: method " + currMethod.mName + " has a postcondition " +
              ...
              if (!canUseForAllExistsRule) {
                if (!isAutoSelected)
                  throw UnknownException(
                    "When using forAllExistsRule, the invariant must satisfy the condition " +
                      "that there are no forall quantifiers over states after an exists quantifier. "
                  )
                else {
                  throw UnknownException(
                    "The forAllExistsRule automatically selected cannot be applied, because at least one of the invariants does not satisfy the condition " +
          ...
          if (!isAutoSelected && rule == "existsRule") {
            ...
            if (!invHasTopExists)
              throw UnknownException(
                "To use the existsRule, at least one of the invariants must contain a top-level " +
          ...
          if (normalizedInvariants.nonEmpty) {
            for ((normalizedInv, i) <- normalizedInvariants.zipWithIndex) {
              ...
              newStmts = newStmts :+ vpr.Assert(translateExp(normalizedInv, null, currStates, loopFailureStates))(info =
                new Logger(VerificationErrors.LoopEntryPoint(oInv), Logger.ERR)
                  .addTitle("Verification Error")
      ...
      case FrameStmt(exp, body) =>
        ...
        val assertFrame = vpr.Assert(framedExpr)(info =
          new Logger(VerificationErrors.HyperAssertion(exp), Logger.ERR)
            .addTitle("Verification Error")
            .addOffset((exp.offsetLeft, exp.offsetRight))
            .toAnnotationInfo()
        )
      ...
      case call @ MethodCallStmt(_, _) =>
        ...
        if (call.method.pre.nonEmpty) {
          ...
          call.method.pre.foreach { precondition =>
            newStmts :+ vpr.Assert(translateExp(precondition, state, currStates, currFailureStates))(info =
              new Logger(VerificationErrors.MethodCall(precondition), Logger.ERR)
                .addTitle("Verification Error")
  ...
  def frameUnmodifiedVars(modifiedVars: Map[String, Type], state: vpr.LocalVar, S1: vpr.LocalVar, S2: vpr.LocalVar, useForAll: Boolean): vpr.Stmt = {
    ...
    val rightExpr = if (modifiedVars.isEmpty) {
      vpr.Exists(
        ...
        Seq.empty,
        vpr.And(
          SetState.getInSetApp(Seq(s_prime.localVar, S2), useForAll),
          vpr.Forall(Seq(vVar), Seq.empty, vpr.EqCmp(State.get(s_prime.localVar, vVarId), State.get(state, vVarId))())()
        )()
      )()
    } else {

      // modifiedVarsVpr is guaranteed to be non-empty
      val modifiedVarsVpr = modifiedVars.map(v =>
        vpr.LocalVar(
          v._1,
          v._2 match {
  ...
  def checkSyncCondModular(normalizedInv: Seq[Expr], body: CompositeStmt, loopGuard: Expr): Boolean = {
    ...
    val allProgVarsInLoopBody = body.allProgVars
      .map(v =>
        vpr.LocalVar(
          v._1,
          v._2 match {
    ...
    val sameGuardValue = vpr.Forall(
      ...
      vpr.Implies(
        ...
        vpr.EqCmp(
          translateExp(loopGuard, s1.localVar, outputStates.localVar, outputFailureStates.localVar),
          translateExp(loopGuard, s2.localVar, outputStates.localVar, outputFailureStates.localVar)
        )()
      )()
    )(info =
      new Logger(VerificationErrors.LoopSyncGuard(loopGuard), Logger.ERR)
        .addTitle("Verification Error")
        .addOffset((loopGuard.offsetLeft, loopGuard.offsetRight))
        .toAnnotationInfo()
    )

    val method = createViperMethod(
      checkSyncCondMethodName, // no update needed
      args,                    // Args
      Seq.empty,
      Seq(pre1) ++ pre2,   // Pres
  ...
  def createViperMethod(methodName: String, args: Seq[vpr.LocalVarDecl], res: Seq[vpr.LocalVarDecl], methodPres: Seq[vpr.Exp], methodPosts: Seq[vpr.Exp], body: Seq[vpr.Stmt], methodLocalVars: Seq[vpr.LocalVarDecl]): vpr.Method = { // NO NEED
    ...
    vpr.Method(methodName, methodArgs, methodRes, methodPres, methodPosts, Option(vpr.Seqn(methodBody, methodLocalVars.map(i => vpr.LocalVarDecl(i.name, i.typ)()))()))()
  }
  ...
  def translateExistsRuleCond1(normalizedInvs: Seq[Expr], loopGuard: Expr, body: CompositeStmt, decrExpr: Expr): vpr.Method = {
    ...
    val varsInStmt = body.allProgVars
      .map(v =>
        vpr.LocalVar(
          v._1,
          v._2 match {
    ...
    posts = pres.map(inv => {
      ...
      (
        inv,
        Option(
          new Logger(VerificationErrors.LoopInvariant(oInv), Logger.ERR)
            .addTitle("Verification Error")
            .addQuantifiersRemoved(qc)
    ...
    val temp            = (
      addToTopExists(firstExistsInv, exprAddedToPost),
      Option(
        new Logger(VerificationErrors.LoopVariant(decrExpr), Logger.ERR)
  ...
  def translateExistsRuleCond2(normalizedInvs: Seq[Expr], loopGuard: Expr, body: CompositeStmt, decrExpr: Expr): vpr.Method = {
    ...
    var varsInStmt = body.allProgVars
      .map(v =>
        vpr.LocalVar(
          v._1,
          v._2 match {
  ...
  def translateInvariantVerificationModular(invs: Seq[Expr], normalizedInv: Seq[Expr], loopGuard: Expr, loopBody: CompositeStmt, decrExpr: Option[Expr], rule: String, isAutoSelected: Boolean): Seq[vpr.Method] = {
    val methodName =
      if (!isAutoSelected) checkInvMethodName + "_" + rule + loopCounter
    ...
    for ((normInv, i) <- normalizedInv.zipWithIndex) {
      ...
      methodPosts = methodPosts :+ translateExp(
        normInv,
        null,
        outputStates,
        outputFailureStates,
        info = new Logger(VerificationErrors.LoopInvariant(oInv), Logger.ERR)
          .addTitle("Verification Error")
          .addQuantifiersRemoved(qc)
          .addWhileRule(rule)
          .addOffset((invs(i).offsetLeft, invs(i).offsetRight))
    ...
    if (decrExpr.isDefined) {
      if (rule == "syncRule" || rule == "forAllExistsRule") {
        ...
        val decrPre        = vpr.Forall(
          Seq(state),
          trigger,
          vpr.Implies(SetState.getInSetApp(Seq(state.localVar, inputStates), useLimited = true), vpr.EqCmp(translatedDecr, State.get(state.localVar, tId))())()
        )(info =
          new Logger(VerificationErrors.LoopVariant(decrExpr.get), Logger.ERR)
    ...
    val allVars = loopBody.allProgVars
      .map(v =>
        vpr.LocalVar(
          v._1,
          v._2 match {
    ...
    val loopGuardHoldsForAll = vpr.Forall(
      Seq(state),
      trigger,
      vpr.Implies(
    ...
    if (rule == "syncRule" || rule == "syncTotRule") {
      if (!isAutoSelected)
        methodBody = methodBody :+ vpr.Assert(sameGuardValue)(info =
          new Logger(VerificationErrors.LoopSyncGuard(loopGuard), Logger.ERR)
            .addTitle("Verification Error")
            .addOffset((loopGuard.offsetLeft, loopGuard.offsetRight))
            .toAnnotationInfo()
    ...
    if (!decrExpr.isEmpty && rule != "syncRule" && rule != "forAllExistsRule") {
      ...
      val decrPost = vpr.Forall(
        Seq(state),
        Seq.empty,
        vpr.Implies(
          SetState.getInSetApp(Seq(state.localVar, outputStates)),
          vpr.And(vpr.GeCmp(translatedDecr, zero)(), vpr.LtCmp(translatedDecr, State.get(state.localVar, tId))())()
        )()
      )(info =
        new Logger(VerificationErrors.LoopVariant(decrExpr.get), Logger.ERR)
  ...
  def translateInvariantVerificationInline(inv: Seq[Expr], loopGuard: Expr, loopBody: CompositeStmt, decrExpr: Option[Expr], currStates: vpr.LocalVar, currFailureStates: vpr.LocalVar, rule: String, isAutoSelected: Boolean): (Seq[vpr.Stmt], Seq[vpr.LocalVar]) = {
    ...
    if (!decrExpr.isEmpty) {
      if (rule == "syncRule" || rule == "forAllExistsRule") {
        ...
        ifBodyStmts = ifBodyStmts :+ vpr.Inhale(vpr.Forall(Seq(state), Seq.empty, vpr.Implies(SetState.getInSetApp(Seq(state.localVar, currStates)), vpr.EqCmp(translatedDecr, State.get(state.localVar, tId))())())())()
    ...
    if (rule == "syncRule" || rule == "syncTotRule") {
      if (!isAutoSelected)
        ifBodyStmts = ifBodyStmts :+ vpr.Assert(sameGuardValue)(info =
          new Logger(VerificationErrors.LoopSyncGuard(loopGuard), Logger.ERR)
            .addTitle("Verification Error")
            .addOffset((loopGuard.offsetLeft, loopGuard.offsetRight))
            .toAnnotationInfo()
    ...
    val assertIs = inv.map { i =>
      vpr.Assert(translateExp(i, null, currStates, currFailureStates))(info =
        new Logger(VerificationErrors.Deprecated(i), Logger.ERR)
          .addTitle("Verification Error")
    ...
    if (decrExpr.isDefined && rule != "syncRule" && rule != "forAllExistsRule") {
      ...
      val assert_variant = vpr.Assert(tf_decr_exp)(info =
        new Logger(VerificationErrors.LoopVariant(decrExpr.get), Logger.ERR)
  ...
  def inhaleInSetEqStmt(state: vpr.LocalVarDecl, currStates: vpr.LocalVar): Seq[vpr.Inhale] = {
    val unlimited = vpr.Inhale(
      vpr.Forall(
        Seq(state),
        Seq.empty,
        vpr.EqCmp(SetState.getInSetApp(Seq(state.localVar, currStates)), SetState.getInSetApp(Seq(state.localVar, currStates), false))()
      )()
    )()

    val limited = vpr.Inhale(
      vpr.Forall(
        Seq(state),
        Seq.empty,
        vpr.EqCmp(SetState.getInSetApp(Seq(state.localVar, currStates), useLimited = true), SetState.getInSetApp(Seq(state.localVar, currStates), false, useLimited = true))()
      )()
    )()
    ...
    e match {
      ...
      case e @ MapAssignExpr(elems) =>
        HHLMap.create(
          ...
          translateType(e.typ.asInstanceOf[MapType].kType),
          translateType(e.typ.asInstanceOf[MapType].vType)
        )
  ...
  def translateHintDecl(decl: HintDecl, arg: vpr.Exp): vpr.Exp = {
    ...
    if (!allFuncsNames.contains(decl.name)) {
      ...
      val hintFunc     = vpr.Function(decl.name, Seq(k), vpr.Bool, Seq.empty, Seq.empty, Option(hintFuncBody))()
  ...
  def translateHavocVarHelper(S1: vpr.LocalVar, S2: vpr.LocalVar, state1: vpr.LocalVar, state2: vpr.LocalVar, varToHavoc: vpr.LocalVarDecl, extraExp: vpr.Exp = null, extraVar: vpr.LocalVarDecl = null, triggers: Seq[vpr.Trigger] = Seq.empty, useForAll: Boolean = true): vpr.Inhale = {
    var itemsInExistsExpr: Seq[vpr.Exp] = Seq(SetState.getInSetApp(Seq(state2, S2), useForAll), State.getEqualExceptApp(Seq(state1, state2, varToHavoc.localVar)))
  ...
  def translateAssumeWithViperExpr(state: vpr.LocalVar, S: vpr.LocalVar, rightExp: vpr.Exp, leftExp: vpr.Exp = null, extraVarDecl: vpr.LocalVarDecl = null, triggers: Seq[vpr.Trigger] = Seq.empty, useForAll: Boolean = true): vpr.Inhale = {
    val lhs = {
